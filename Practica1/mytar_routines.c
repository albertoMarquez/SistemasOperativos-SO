...
int extractTar(char tarName[])
{
	...
	//Abrimos el fichero, probar si falla con la opcion r
	if ((tarFile = fopen(tarName, "r")) == NULL) {
		printf("El archivo mtar %s no ha podido ser abierto: ", tarName);
		remove(tarName);
		return (EXIT_FAILURE);
	}

	if((header = readHeader(tarFile, &nFiles)) == NULL){
		printf("No se ha podido cargar la cabecera\n");
		remove(tarName);
		fclose(tarFile);
		return (EXIT_FAILURE);
	}
	...
	for(int i = 0; i < nFiles; i++){
		if ((file = fopen(header[i].name, "wb")) == NULL || (copynFile(tarFile, file, header[i].size) == -1)) {
	...
	printf("Archivos extraidos con exito\n");
	fclose(tarFile);
	free(file);
	for(int i = 0; i < nFiles; i++)
		free(header[i].name);
...
int extractReverseTar(char tarName[]) {
	...
	fread(&totalSize, sizeof(unsigned int), 1, tarFile);
	fseek(tarFile, totalSize, SEEK_SET);

	if((header = readReverseHeader(tarFile)) == NULL){
		printf("No se ha podido cargar la cabecera\n");
		remove(tarName);
		fclose(tarFile);
		for(int i = 0; i < nFiles; i++)
